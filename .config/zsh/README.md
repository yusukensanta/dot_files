...
|---|---|---|---|
| `00-09` | Environment: PATH, umask, XDG dirs | `00-env.zsh` | **Must precede `02-plugins.zsh` — tested** (fresh-`$HOME` run: `02`'s zsh-abbr loop reads `DOTFILES_BREW_PREFIXES`, defined here; without it, `abbr` never loads at all). *Not* shown to need to be absolute-first: on this machine mise/starship/fzf/fd/rg/zoxide were already reachable without this file's PATH additions — that's this machine's inherited PATH being complete already, not a guarantee, so don't rely on it |
